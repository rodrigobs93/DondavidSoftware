# Don David POS — Windows Installer

One-click installation + one-click run for Don David POS on Windows 10/11 x64.
...
|-----------|----------|----------|
| PHP 8.2 (non-thread-safe, CLI) | yes | `C:\DonDavid\php\` |
| PostgreSQL 16 (portable) | yes | `C:\DonDavid\pgsql\` — runs as Windows service `DonDavidPostgres` |
| Laravel app (vendor\ pre-installed) | yes | `C:\DonDavid\app\` |
| Composer (for future in-place upgrades) | yes | `C:\DonDavid\composer.phar` |
| Launcher scripts | yes | `C:\DonDavid\scripts\` |
...
3. Invoke Inno Setup Compiler to produce
   `installer\output\DonDavidSetup-<version>.exe` (~250–350 MB).
...
### Release an update
Increase `#define AppVersion` in `DonDavid.iss`, commit, then rebuild and copy
...
2. Copy `DonDavidSetup-<version>.exe` to the target PC.
3. Right-click → Run as administrator (or double-click and approve UAC).
4. Follow the wizard:
   - **Install location**: keep default `C:\DonDavid`.
...
5. Wait for "Setup completed successfully". A **Don David POS** icon appears on
   the desktop.
...
Double-click **Don David POS** on the desktop. The launcher:
1. Acquires an exclusive lock (no double-boot even if you click 10× fast).
...
### Logs
- `C:\DonDavid\logs\launcher.log` — one line per run of `start.ps1`.
- `C:\DonDavid\logs\laravel-YYYY-MM-DD.log` — Laravel server stdout/stderr.
- `C:\DonDavid\logs\worker-YYYY-MM-DD.log`  — print worker stdout/stderr.
- `C:\DonDavid\logs\install.log` — full transcript of the install script.

---

## Uninstall

Control Panel > Programs → select **Don David POS** → Uninstall. This:
- Stops Laravel + Worker (`stop.ps1`)
- Stops and unregisters the `DonDavidPostgres` service
- Removes the firewall rule
- Removes the desktop + startup shortcuts
- Removes `C:\DonDavid\` (including `pgsql\data\` — **your data is deleted**;
...
| Symptom | First check |
|---------|-------------|
| Icon click does nothing | Open `C:\DonDavid\logs\launcher.log`; most recent line shows where it bailed |
| Browser opens but "can't reach" | Laravel failed to start — see `logs\laravel-<today>.log` |
| Browser says DB error | Postgres not running — `Get-Service DonDavidPostgres` |
